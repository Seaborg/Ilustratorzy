class PicsController < ApplicationController
  ...
  def index
    ...
    @users = User.limit(20)
    @usersSideList = User.all(:order => 'reputation DESC', :limit => 18)
  ...
  def create
    ...
    respond_to do |format|
      if @pic.save
        flash[:notice] = 'Grafika została utworzona.'
        format.html { redirect_to(@pic) }
  ...
  def update
    respond_to do |format|
      if @pic.update_attributes(params[:pic])
        flash[:notice] = 'Grafika została zaktualizowana poprawnie.'
        format.html { redirect_to(@pic) }
        format.xml  { head :ok }
      else
...
private
  ...
  def current_user_must_own_pic
    ...
    if current_user && (current_user.owns_pic?(@pic) || current_user.is_admin)
      ...
      flash[:notice] = 'Możesz edytować tylko własne prace.'
      redirect_to :back
    end
  end
