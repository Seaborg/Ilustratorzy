class User < ActiveRecord::Base
    ...
    validates_attachment_content_type :avatar, :content_type=>['image/jpeg','image/pjpeg','image/png','image/x-png'], :message=>'Format grafiki może być tylko .jpeg or .png'
    validates_attachment_size :avatar, :less_than => 2.megabytes, :message=>'Grafika może mieć tylko 2 MB albo mniej'       
    ...
    validates_format_of :login, :with => /^\w+$/i, :message => "Twój login może zawierać tylko litery i cyfry."
    ...
    def heart_pic(pic)
       if self.hearts_left > 0
          ...
          if heart.save
            ...
            if self.save
              return 0 #flash[:notice] = "Grafika z kredkami."
            else
              return 1 #flash[:notice] = "Błąd dodania kredek."
            end
          else
            return 1 #flash[:notice] = "Błąd przyznawania kredek."
          end
        else
          return 2 #flash[:notice] = "Nie pozostało Ci więcej kredek. Kliknij na 'Kredki' w górnym menu"
        end
    end
